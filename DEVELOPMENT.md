...
This project pins the supported release-verification toolchain to Python `3.11.x`, Node.js `22.18.0`, and npm `10.9.3` for `1.6.0`.
...
- Complete and abandon flows should use the same accessible confirmation-dialog pattern; `window.confirm()` is no longer sufficient for release-quality UI behavior.
...
- Demo reset does not pre-seed fake Learning Session rows, event timelines, or learner-authored goals for `v1.6.0`; those remain runtime-only data after startup.

Version verification note:

- `scripts/verify_delivery.py` uses strict release-facing version checks for `1.6.0`, including root/frontend/package-lock parity plus the canonical README, release checklist, and demo-guide markers.
...
- `1.6.0` has passed validation on Node.js `22.18.0` and npm `10.9.3`, including full frontend reinstall, E2E, Docker validation, and delivery verification.
