...
- Review `CHANGELOG.md` and confirm the release-facing notes for `v1.6.0`.
- Update root `VERSION`; it is the source of truth for backend app metadata and release archives. Keep `frontend/package.json` in sync at `1.6.0`; `scripts/verify_delivery.py` checks this.
...
- Explicitly verify Learning Session release fixes: abandon state consistency, nullable `weekly_minutes`, Session history pagination, event timeline pagination, confirmation dialog cancellation/submit behavior, and recorder degraded readiness exposure.
...
- Confirm `RELEASE_NOTES_v1.6.0.md` exists and matches the shipped API, migration, and runtime behavior. Keep `RELEASE_NOTES_v1.6.0-rc1.md` as historical RC documentation.
...
- Do not tag or publish `1.6.0` until the canonical Python `3.11.x`, Node `22.18.0`, and npm `10.9.3` toolchain has passed the full verifier, Docker, frontend reinstall/audit, mocked E2E, full-stack E2E, and delivery gates for the final release state.
