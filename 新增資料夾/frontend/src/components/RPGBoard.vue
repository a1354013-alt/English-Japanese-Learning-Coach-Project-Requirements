    ...
    <div class="flex flex-col md:flex-row items-center gap-8 relative z-10">
      ...
      <div class="flex-1 w-full">
        <div class="flex justify-between items-end mb-2">
          <div>
            ...
            <p class="text-slate-400 text-sm">目標：{{ stats.target_goal || 'TOEIC 800 (範例)' }}</p>
          </div>
          <div class="text-right">
            <span class="text-sm text-blue-400 font-mono">{{ stats.current_xp }} / {{ stats.next_level_xp }} XP</span>
          </div>
        </div>
        ...
        <div class="h-4 w-full bg-slate-800/50 rounded-full overflow-hidden border border-white/5">
          <div 
            ...
            :style="{ width: `${(stats.current_xp / stats.next_level_xp) * 100}%` }"
          ></div>
        </div>
      </div>
      ...
      <div class="flex gap-4">
        ...
        <div class="glass-panel p-3 rounded-xl text-center min-w-[80px] hover-glow">
          ...
          <div class="text-lg font-bold text-white">{{ stats.achievements.length }}</div>
          <div class="text-[10px] text-slate-400 uppercase tracking-wider">Achievements</div>
        </div>
      </div>
    </div>
  </div>
...
import type { UserRPGStats } from '../types'
